...
//   draw();
// });
colorModeButtonDOM.addEventListener("touchstart", () => {
  if (settings.mode === "dark") {
    settings.mode = "light";
    colorModeButtonDOM.innerText = "Dark Mode";
  } else {
...
bombGrabAreaDOM.addEventListener("touchstart", function (e) {
  ...
  if (state.phase === "aiming") {
    isDragging = true;
    dragStartX = touches.clientX;
    dragStartY = touches.clientY;
  }
});
window.addEventListener("touchmove", function (e) {
  let touches = e.touches[0]
  if (isDragging) {
    let deltaX = touches.clientX - dragStartX;
    let deltaY = touches.clientY - dragStartY;
...
window.addEventListener("touchend", function (event) {
  if(event.target != bombGrabAreaDOM){
    if (isDragging) {
      isDragging = false;
